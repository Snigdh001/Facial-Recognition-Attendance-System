    ...
    def delete_data(self):
        if self.var_enroll.get()=="":
            ...
            try:
                ...
                del_img_id=1
                while del_img_id<=100:
                    conn1=mysql.connector.connect(host="localhost", user="root", password="snigdh", database="face_recognition")
                    my_cursor1=conn1.cursor()
                    my_cursor1.execute("select Enrollment from student where Enrollment=%s ",(self.var_enroll.get(),))
                    en=str(my_cursor1.fetchone())[8:-3]
                    file="Data/user."+en+"."+str(del_img_id)+".jpg"
                    del_img_id+=1
                    os.remove(file)
                    conn1.commit()
                    conn1.close()
